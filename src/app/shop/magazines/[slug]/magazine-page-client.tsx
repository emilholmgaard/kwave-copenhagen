...
export function MagazinePageClient({ magazine }: { magazine: Magazine }) {
  ...
  const displayImage = magazine.imageShort || magazine.imageLong || magazine.image
  const hasMultipleImages = magazine.imageShort && magazine.imageLong
      ...
      <Container>
        ...
        <div className="mt-16 grid grid-cols-1 gap-12 pb-24 lg:grid-cols-2 lg:items-start relative" style={{ zIndex: 1 }}>
          <div className="flex flex-col gap-4">
            {hasMultipleImages ? (
              <>
                <div className="flex items-center cursor-pointer" onClick={() => setFullscreenImage(magazine.imageShort!)}>
                  <img
                    alt={magazine.title}
                    src={magazine.imageShort}
                    className="w-full rounded-3xl shadow-2xl ring-1 ring-black/5 transition-transform hover:scale-[1.02]"
                  />
                </div>
                <div className="flex items-center cursor-pointer" onClick={() => setFullscreenImage(magazine.imageLong!)}>
                  <img
                    alt={magazine.title}
                    src={magazine.imageLong}
                    className="w-full rounded-3xl shadow-2xl ring-1 ring-black/5 transition-transform hover:scale-[1.02]"
                  />
                </div>
              </>
          ...
          <div className={`flex flex-col relative z-10 ${hasMultipleImages ? 'lg:sticky lg:top-24 lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto lg:overflow-x-visible lg:pb-8 lg:px-2' : 'justify-center'}`}>
            <Subheading>{magazine.artist}</Subheading>
            <Heading as="h1" className="mt-2">
              {magazine.title}
            </Heading>
            <div className="mt-6 flex items-center gap-4">
              <div className="text-4xl font-semibold text-white">
                {magazine.price.toFixed(2).replace('.', ',')} {magazine.currency}
              </div>
              {magazine.isPreOrder && (
            ...
            <p className="mt-6 text-base/7 text-gray-300">{magazine.description}</p>
            <div className="mt-8 flex flex-wrap items-center gap-4">
              <div>
                ...
                <span className="text-sm text-gray-400">
                  {dayjs(magazine.releaseDate).format('MMMM D, YYYY')}
                </span>
              </div>
              <div>
                <span className="text-sm font-medium text-white">
            ...
            {magazine.features && magazine.features.length > 0 && (
            ...
            <div className="mt-8">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-sm text-gray-400">
                  Total: <span className="font-semibold text-white">{(magazine.price * quantity).toFixed(2).replace('.', ',')} {magazine.currency}</span>
                </div>
              </div>
            </div>
