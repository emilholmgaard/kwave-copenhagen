...
export function DVDCard({ dvd }: { dvd: DVD }) {
  return (
    <Link
      href={`/shop/dvds/${dvd.slug}`}
    ...
    >
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-2 text-xs text-gray-400">
          Release: {dayjs(dvd.releaseDate).format('MMM D, YYYY')}
        </div>
          ...
          {dvd.inStock || dvd.isPreOrder ? (
            <div className="flex gap-2">
              <AddToCartButton 
                albumId={dvd.id} 
                quantity={1}
                disabled={!dvd.inStock && !dvd.isPreOrder}
                type="dvd"
              />
              <Button variant="outline" href={`/shop/dvds/${dvd.slug}`}>
                Buy Now
              </Button>
            </div>
          ) : (
            <Button href={`/shop/dvds/${dvd.slug}`} variant="outline" className="gap-2">
